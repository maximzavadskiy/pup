...
const EngagedCandidatesList = ({ loading, matchedUsers, match, history }) =>
    !loading ? (
            ...
            <Grid>
                {matchedUsers.length ? (
                        ...
                        ({ _id, profile, createdAt, updatedAt }) => (
                            <Row key={_id}>
                                <Col xs={12}>
                                    ...
                                    >
                                        {profile.teammateChats &&
                                        profile.teammateChats.indexOf(
                                            Meteor.user()._id
                                        ) !== -1 ? (
                                            <p>
                                                {' '}
                                                You & the user initiated Chat
                                                with each other!{' '}
                                            </p>
                                        ) : (
                                            <p>
                                                {' '}
                                                One of you have starred another
                                                - drop a message and see what
                                                you can do together!{' '}
                                            </p>
                                        )}
                                    </Panel>
                    ...
                    <Alert bsStyle="warning">
                        Here will be displayed all people whom you contacted, or
...
export default withTracker(() => {
    ...
    const likedByUser = Meteor.users
        .find({
            $or: [
